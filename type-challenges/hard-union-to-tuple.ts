/**
 * 题目：https://github.com/type-challenges/type-challenges/blob/main/questions/00730-hard-union-to-tuple/README.md
 *
 *
 * 思路：
 * 1.
...
// 参考实现：
// https://github.com/type-challenges/type-challenges/issues/10191 有详细解释
...
type UnionToFuncIntersec<T> = (T extends any ? (X: () => T) => void : never) extends (
  P: infer R
) => void
  ? R
  : never;

// 函数联合类型 和 函数重载一样，保留最后一个
type GetLastUnionOne<T> = UnionToFuncIntersec<T> extends { (): infer R } ? R : never;

type UnionToTuple<T, U = GetLastUnionOne<T>> = [T] extends [never]
  ? []
